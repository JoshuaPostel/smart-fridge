...
def plot_refrigerator(
    ...
    par2.set_ylim(-max(cumulative_co2) * 0.05, max(cumulative_co2) * 1.05)
    par3.set_ylim(0, 50)

    host.set_xlabel("Date")
    host.set_ylabel("Temperature F°")
    par1.set_ylabel("MOER")
    par2.set_ylabel("Cumulative CO2")
    ...
    p3, = par2.plot(times, cumulative_co2, color=color3, label="Cumulative CO2")
